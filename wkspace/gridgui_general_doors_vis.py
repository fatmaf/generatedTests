    ...
    def draw(self):
        ...
        if self.master != None :
            ...
            if self.isInitPos:
                ...
                ypos = ymin+crad
                d = (self.abs,self.ord)
                self.master.create_oval(xpos,ypos,xpos+crad,ypos+crad,fill=Cell.robot_colors[d],outline=Cell.BORDER_COLORS[GuiState.INITLOCS])
            if self.isDoor:
                d = (self.abs,self.ord)
                if d[0] == 1:
                ...
                #elif d[0] == 2:
                #    self.master.create_line(xmin,ymin+self.size/4,xmin+self.size/4,ymax-self.size/3)
                #if self.otherDoor is not None:
                    #od = self.otherDoor
                    #print (od)
                    #print (d)
                #print (d)
                #self.master.create_line(xmax,ymin,xmax-self.size/4,ymax)
            #if self.isGoalPos:
            #    xpos = xmin+crad
            #    ypos = ymin+crad
            #    starLen = crad
            #    starWidth = crad/2
            #    starPoints = self.getStarPoints(xpos,ypos,starLen,starWidth)
            #    self.master.create_polygon(starPoints,fill = Cell.FILL_COLORS[GuiState.GOALS],outline=Cell.BORDER_COLORS[GuiState.GOALS],width=1)
...
class GridGui(object):
    ...
    def moveRobots(self):
        ...
        print "move robots not implemented"
    ...
    def createGuiMenu(self):
        ...
        self.menu.add_command(label='Move', command=self.moveRobots)
